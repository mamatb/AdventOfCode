...
func getRegion(garden [][]string, inRegion map[position]bool,
	...
	for len(posPending) > 0 {
		...
		if !inRegion[pos] && garden[pos.row][pos.col] == label {
			...
			for _, posNeighbour := range getNeighbours(pos) {
				if posInMap(posNeighbour, rows, cols) {
					posPending = append(posPending, posNeighbour)
				}
			}
		}
	}
...
func getPrice(region map[position]bool) int {
	regionArea, regionPerim := len(region), 0
	for pos := range region {
		regionPerim += 4
		for _, posNeighbour := range getNeighbours(pos) {
			if region[posNeighbour] {
				regionPerim -= 1
			}
		}
	}
